...
module.exports = function replaceSvgWith({
  ...
  console.log('running replaceSvgWith')
  ...
  function replaceElement(path, state) {
    ...
    const getAttributeValue = ({
      ...
      if (literal === 'props') {
        return t.JSXExpressionContainer(template.ast(`props.${key}`).expression);
    ...
    const convertToExpressions = value => typeof value === 'object' ? convertObjectToExpression(value) : convertToPropsExpression(value); // Replace element by react-native-svg components
    // const component = elementToComponent[name]
    // const openingElementName = path.get('openingElement.name');


    const attrs = (attributes && attributes.length ? attributes.map(convertToExpressions) : [convertToSpreadAttributes('props')]).filter(t => !!t);
    path.replaceWith(t.JSXElement(t.JSXOpeningElement(t.JSXIdentifier(componentName), attrs), t.JSXClosingElement(t.JSXIdentifier(componentName)), path.node.children));
    return;
  }
